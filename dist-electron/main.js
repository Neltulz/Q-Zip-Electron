...
function setup7ZipHandlers() {
  ...
  const ensureSessionRoot = () => {
    const fs2 = require$1("fs");
    const os = require$1("os");
    const pathLocal = require$1("path");
    if (sessionRoot && typeof sessionRoot === "string" && fs2.existsSync(sessionRoot)) return sessionRoot;
    sessionRoot = fs2.mkdtempSync(pathLocal.join(os.tmpdir(), "qzip-staging-"));
    return sessionRoot;
  };
  const cleanupSessionRoot = () => {
    try {
      ...
      const fs2 = require$1("fs");
      if (fs2.existsSync(sessionRoot)) {
        fs2.rmSync(sessionRoot, { recursive: true, force: true });
      }
      sessionRoot = null;
    } catch (e) {
...
function createWindow() {
  ...
  const preloadCjs = path$1.join(preloadDir, "preload.cjs");
